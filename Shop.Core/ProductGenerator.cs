    ...
    public class ProductGenerator : IProductGenerator
    {
        ...
        public List<Product> GenerateList()
        {
            ...
            foreach (Model.Products.Product product in products)
            {
                try
                {
                     // Jako ze product nie zawier Id, to sobie wygenerujemy Id, przydatne bo Shop.Core moze robic jakies obliczenia, wiec normalne ze MOdel moze nie miec jakiegos property
                    ...
                    if (productDTO.Id >= 500 && productDTO.Id <= 1500
                    ) // jesli id pomiedzy 500 (wliczajac 500) a 15000 (wliczajac 1500) 
                        productDTO.Type = "Monitor"; //To Type = Monitor
                    if (productDTO.Id > 1500) // jesli Id > 1500
                        productDTO.Type = "Trash"; // To produkt to smiec :D

                    productList.Add(
                        productDTO); // teraz mamy obiekt ktory wypelnione ma wszystkie properties, jakas logike, i ten sam typ co productList (nie mozna dodawac dowolnych rzeczy do Listy danego typu. Musi byc ten sam typ
                }
                catch (Exception e)
                {
        ...
        public async Task<List<Product>> GenerateListAsync() // dodano async Task
        {
            var
                products = await _productRepo
                    .GetProductsAsync(); // teraz mozesz uzyc GetProductsAsync z await, bo bez await bedzie obiekt typu Task a nie IList jako rezultat. Await to jakby metoda Task ktora mowi ze ma zwrocic type z metody, a nie Task
            ...
            foreach (Model.Products.Product product in products)
            {
                ...
                {
                    Random
                        rand = new Random(); // Jako ze product nie zawier Id, to sobie wygenerujemy Id, przydatne bo Shop.Core moze robic jakies obliczenia, wiec normalne ze MOdel moze nie miec jakiegos property
                    //Poza tym mozesz laczyc np. product z zamowieniem i mixowac do DTO ktore jest uzywane gdzie indziej
                    ...
                    productDTO.Name =
                        product.Name +
                        $" [{productDTO.Id}]"; // Tworze name na podstawie product.name + dodaje [id], wiec bedzie Trash [3]
